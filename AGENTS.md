...
A school staff web tool that extracts teacher initials from uploaded timetable PDFs, matches them against a staff directory, lets staff review and correct uncertain matches, then exports a clean four-column teacher-contact table for parent letters.
...
- **Commit as your GitHub identity, or Vercel will block the deploy.** Vercel verifies that
  ...
  ```
  git config user.email "241361952+Ngphebee9468@users.noreply.github.com"
  git config user.name "Ngphebee9468"
  ```
